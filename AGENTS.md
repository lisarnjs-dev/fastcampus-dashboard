...
기능 추가, 개선, 버그 수정 작업을 완료할 때마다 **`docs/CHANGELOG.md`** 맨 위(최신순)에 항목을 추가한다.

- 날짜별 섹션 아래 **기능 추가 / 개선 / 버그 수정 / 문서 / 테스트** 분류로 기록
- 버그 수정은 원인과 해결 방법을 함께 기록
- 변경 이력은 이 문서 하나로만 관리한다 (별도 문서 분산 금지)
